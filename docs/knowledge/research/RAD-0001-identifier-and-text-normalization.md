...
**Provider-specific rules were considered and rejected.** Gmail treats dots in the local part as insignificant, and folding them would genuinely improve matching for that provider. Three things killed it. It is non-standard, so nothing obliges the provider to keep behaving that way. It is unknowable in general — there is no way to enumerate which of the world's mail hosts do what. And a frozen provider list could never *grow*: adding a provider later changes the canonical bytes for every address at that domain, splitting historical tokens from new ones. The line that survived is **a universal standard, yes; a single-provider behaviour, no** — which is why removing an RFC 5233 `+`-subaddress is offered as an option a caller asks for, and folding dots is not offered at all. Subaddressing is at least a standard, so a caller who knows their provider honours it can ask; dots are one provider's behaviour and nothing in the suite can act on them.
...
`0.0.4` renames **every** policy id: links join with `:` rather than `+`, names drop their hyphens, and a link that acts on the value reads `<subject>.<what was done>` — `text.u17:space.trimmed:case.folded:nfc`, `ipv4.quad.dotted:block.24`. One behaviour changes with it: the email base keeps the `+`-subaddress and removing it becomes the option `email:subaddress.removed`, because no domain can be asked whether a tag is part of an account. Bytes are unchanged throughout, `0.0.3` ids are withdrawn rather than aliased, and it was affordable only because both known consumers confirmed they held no stored tokens. See aughtone/aughtone-normalize#29.
