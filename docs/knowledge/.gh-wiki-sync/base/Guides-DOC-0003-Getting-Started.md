...
Read [Normalization Suite Structure](Specifications-DOC-0001-Normalization-Suite) before touching anything that can reach a canonical string. [Kotlin Multiplatform Conventions](Specifications-DOC-0002-Kmp-Conventions) covers the multiplatform traps that are not obvious from a single-target build.
