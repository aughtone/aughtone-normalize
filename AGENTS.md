...
- **KDoc is part of the change, not a follow-up.** Whenever you modify code, check the KDoc that describes it and update it in the same change — the declaration you touched, the function that returns it, the type it is a field of, and anything that links to it. This is a published library: the KDoc is what a consumer reads at the moment they write the call, so a stale line there is worse than a stale line in the README, which they read once. Check for four things specifically, because these are the ones that survive a rename and go on lying: a sentence describing behaviour that changed; a sample in a `` ``` `` block that no longer compiles or no longer shows the current signature; a policy id, link name or error type spelled the way it used to be; and a claim about what a call refuses, which changes the moment a new failure can reach it. If the code no longer does what the KDoc says, the KDoc is a defect — fix it or say why you did not.
