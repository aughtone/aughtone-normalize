...
class EmailByteStabilityTest {
    ...
    fun refusalsAreFrozen() {
        val refusals = listOf<Pair<String, (Throwable) -> Boolean>>(
            ...
            // Built from code units, not written as literals: a lone surrogate in source is corrupted
            // to U+FFFD by the JS bundler, which would leave this expectation passing while testing
            // nothing on that target. The expectation itself is unchanged - these inputs are refused.
            ("user" + Char(0xD83D) + "@example.com") to { it is EmailNormalizationError.UnpairedSurrogate },
